...
class EdgeDataset:
    ...
    def __init__(self, g: nx.MultiDiGraph, cfg: RunConfig) -> None:
        ...
        rows: list[tuple[int, str, str, float, int, list[float]]] = []
        for u, v, d in g.edges(data=True):
            eid = int(d["edge_id"])
            rows.append((eid, u, v, float(d["t"]), int(d["is_laundering"]), feats[eid]))
        ...
        self.X = np.array([r[5] for r in rows], dtype=np.float32)
