...
|---|:---:|:---:|:---:|:---:|
| **LightGBM (мотивы)** | **0.421** | **0.397** | **1.00** | **92.0 %** |
| Hybrid (rank-fusion 50/50) | 0.343 | 0.359 | 1.00 | 90.3 % |
| GNN (GraphSAGE) | 0.267 | 0.269 | 0.97 | 87.1 % |
...
признаков. Гибрид (равновесное слияние рангов, веса фиксированы заранее — ничего не
подгонялось) ложится между компонентами. Признаки строго каузальны (`amount_burst`
считается только по прошлой истории отправителя), поэтому вся схема воспроизводима в
проде. Топ-100 алертов LightGBM — целиком настоящие: поток на ручной разбор сокращается
на ~92 %.
...
| Документ | Что внутри |
|---|---|
| [`docs/beginner_guide.md`](docs/beginner_guide.md) | 🐣 **гид для новичка**: проект простыми словами, словарик всех аббревиатур, ML «на пальцах» |
